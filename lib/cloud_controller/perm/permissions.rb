  ...
  module Perm
    class Permissions
      ...
      def readable_route_guids
        if can_read_globally?
          CloudController::Route.select(:guid).all.map(&:guid)
        else
      ...
      def readable_app_guids
        if can_read_globally?
          CloudController::AppModel.select(:guid).all.map(&:guid)
        else
      ...
      def readable_route_mapping_guids
        if can_read_globally?
          CloudController::RouteMappingModel.select(:guid).all.map(&:guid)
        else
