...
Sequel.migration do
  up do
    self[:apps].each do |row|
      ...
      encrypted = CloudController::Encryptor.encrypt(row[:environment_json], salt)
  ...
  down do
    ...
    self[:apps].each do |row|
      decrypted = CloudController::Encryptor.decrypt(row[:encrypted_environment_json], row[:salt])
