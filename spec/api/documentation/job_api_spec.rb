...
RSpec.resource 'Jobs', type: [:api, :legacy_api] do
  ...
  get '/v2/jobs/:guid' do
    ...
    describe 'When a job has failed with a known failure from v2.yml' do
      before { CloudController::Jobs::Enqueuer.new(KnownFailingJob.new).enqueue }
    ...
    describe 'When a job has failed with an unknown failure' do
      ...
      before { CloudController::Jobs::Enqueuer.new(UnknownFailingJob.new).enqueue }
    ...
    describe 'For a queued job' do
      ...
      before { CloudController::Jobs::Enqueuer.new(SuccessfulJob.new).enqueue }
