...
RSpec.resource 'Spaces', type: [:api, :legacy_api] do
  ...
  describe 'Standard endpoints' do
    ...
    def after_standard_model_delete(guid)
      event = CloudController::Event.find(type: 'audit.space.delete-request', actee: guid)
      audited_event event
    end
    ...
    post '/v2/spaces/' do
      ...
      example 'Creating a Space' do
        organization_guid = CloudController::Organization.make.guid
        ...
        audited_event CloudController::Event.find(type: 'audit.space.create', actee: space_guid)
      end
    end
    ...
    put '/v2/spaces/:guid' do
      ...
      example 'Update a Space' do
        ...
        audited_event CloudController::Event.find(type: 'audit.space.update', actee: guid)
      end
    end
  end
  ...
  describe 'Nested endpoints' do
    ...
    before do
      CloudController::SecurityContext.set(user, 'valid_token')
    end
    ...
    describe 'Routes' do
      before do
        domain = CloudController::PrivateDomain.make(owning_organization: space.organization)
        CloudController::Route.make(domain: domain, space: space)
      end
    ...
    describe 'Developers' do
      before do
        ...
        allow_any_instance_of(CloudController::UaaClient).to receive(:usernames_for_ids).and_return({ associated_developer.guid => 'developer@example.com' })
      end

      let!(:associated_developer) { CloudController::User.make }
      let(:developer) { CloudController::User.make }
    ...
    describe 'User Roles' do
      ...
      before do
        ...
        allow_any_instance_of(CloudController::UaaClient).to receive(:usernames_for_ids).and_return({ everything_user.guid => 'everything@example.com' })
      end
    ...
    describe 'Apps' do
      before do
        CloudController::ProcessModelFactory.make(space: space)
      end

      standard_model_list 'ProcessModel', CloudController::AppsController, path: :app, outer_model: :space
    end
    ...
    describe 'Service Instances' do
      before do
        CloudController::ManagedServiceInstance.make(space: space)
      end
    ...
    describe 'Events' do
      before do
        ...
        space_event_repository = CloudController::Repositories::SpaceEventRepository.new
        space_event_repository.record_space_update(space, CloudController::UserAuditInfo.new(user_guid: user.guid, user_email: 'user@example.com'), { 'name' => 'new_name' })
      end
    ...
    describe 'Security Groups' do
      let!(:associated_security_group) { CloudController::SecurityGroup.make(space_guids: [space.guid]) }
    ...
    describe 'Isolation Segments (experimental)' do
      ...
      let(:org_manager) { CloudController::User.make }
      ...
      before do
        ...
        allow_any_instance_of(CloudController::UaaClient).to receive(:usernames_for_ids).and_return({ org_manager.guid => 'manager@example.com' })
      end
