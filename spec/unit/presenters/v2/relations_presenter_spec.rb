...
module CloudController::Presenters::V2
  RSpec.describe RelationsPresenter do
    describe '#to_hash' do
      let(:test_model) { CloudController::TestModel.make(unique_value: 'something', required_attr: true) }
      let(:opts) { {} }
      ...
      it 'includes urls for each related association on the object' do
        hash = subject.to_hash(CloudController::TestModelsController, test_model, opts, 0, [])
        expect(hash).to eql(
          {
      ...
      describe 'to_one relationship' do
        let(:test_model_many_to_one) { CloudController::TestModelManyToOne.make }

        it 'includes to_one relationship link when association is set' do
          test_model_many_to_one.test_model = test_model
          hash = subject.to_hash(CloudController::TestModelManyToOnesController, test_model_many_to_one, opts, 0, [])
        ...
        it 'raises NotLoadedAssociationError when association is not loaded and will be included so that this object is not responsible for checking authorization' do
          expect {
            subject.to_hash(CloudController::TestModelManyToOnesController, test_model_many_to_one, opts, 0, [])
          }.to raise_error(CloudController::Errors::NotLoadedAssociationError, /test_model.*CloudController::TestModelManyToOne/)
        end
      end

      describe 'to_many relationship' do
        let(:test_model_many_to_many) { CloudController::TestModelManyToMany.make }
        ...
        it 'includes the association content when link_only is not specified' do
          test_model.add_test_model_many_to_many(test_model_many_to_many)
          hash = subject.to_hash(CloudController::TestModelsController, test_model, opts.merge(inline_relations_depth: 1), 0, [])
        ...
        it 'raises NotLoadedAssociationError when association is not loaded and will be included so that this object is not responsible for checking authorization' do
          ...
          test_model.remove_all_test_model_many_to_ones
          expect {
            subject.to_hash(CloudController::TestModelsController, test_model, opts.merge(inline_relations_depth: 1), 0, [])
          }.to raise_error(CloudController::Errors::NotLoadedAssociationError, /test_model_many_to_manies.*CloudController::TestModel/)
        end
      end

      it 'serializes related inline objects inline' do
        test_model_many_to_many = CloudController::TestModelManyToMany.make
        test_model_second_level = CloudController::TestModelSecondLevel.make
        ...
        hash = subject.to_hash(CloudController::TestModelsController, test_model, opts.merge(inline_relations_depth: 2), 0, [])
      ...
      describe 'orphan_relations enabled' do
        it 'serializes related n:many inline objects as orphans' do
          ...
          orphans = {}
          hash = subject.to_hash(CloudController::TestModelsController, test_model, opts.merge(inline_relations_depth: 2), 0, [], orphans)
        ...
        it 'serializes related n:one inline objects as orphans' do
          test_model_many_to_one = CloudController::TestModelManyToOne.make
          test_model_many_to_one.test_model = test_model

          orphans = {}
          hash = subject.to_hash(CloudController::TestModelManyToOnesController, test_model_many_to_one, opts.merge(inline_relations_depth: 1), 0, [], orphans)
      ...
      it 'excludes relations named in exclude_relations' do
        ...
        hash = subject.to_hash(CloudController::TestModelsController, test_model, opts.merge(inline_relations_depth: 2, exclude_relations: 'test_model_second_levels'), 0, [])
        expect(hash.fetch('test_model_many_to_manies')).to eql([
          'metadata' => {
            'guid' => test_model_many_to_many.guid,
            'url' => "/v2/test_model_many_to_manies/#{test_model_many_to_many.guid}",
      ...
      it 'only includes relations named in include_relations' do
        ...
        hash = subject.to_hash(CloudController::TestModelsController,
          test_model,
