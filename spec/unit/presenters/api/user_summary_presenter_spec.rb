...
RSpec.describe UserSummaryPresenter do
  describe '#to_hash' do
    ...
    let(:managed_org) { CloudController::Organization.make }
    let(:billing_managed_org) { CloudController::Organization.make }
    let(:audited_org) { CloudController::Organization.make }
    let(:space) { CloudController::Space.make(organization: org) }
    let(:managed_space) { CloudController::Space.make(organization: managed_org) }
    let(:audited_space) { CloudController::Space.make(organization: audited_org) }
    let(:user) do
