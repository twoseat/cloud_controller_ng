      ...
      describe '#to_hash' do
        ...
        it 'sends false for show_secrets' do
          ...
          expect(MonkeyPresenter).to have_received(:new).
            with(anything, show_secrets: false, censored_message: Censorship::REDACTED_LIST_MESSAGE).exactly(set.count).times
        end
        ...
        context 'when show_secrets is true' do
          ...
          it 'sends true for show_secrets' do
            ...
            presenter.to_hash
            expect(MonkeyPresenter).to have_received(:new).
              with(anything, show_secrets: true, censored_message: Censorship::REDACTED_LIST_MESSAGE).exactly(set.count).times
          end
        end
