...
module VCAP::Services::SSO::Commands
  RSpec.describe UnclaimClientCommand do
    ...
    describe '#db_command' do
      before do
        allow(CloudController::ServiceDashboardClient).to receive(:release_client)
      end

      it 'claims the client in the DB' do
        command.db_command
        expect(CloudController::ServiceDashboardClient).to have_received(:release_client).with(client_id)
      end
    end
  end
end
