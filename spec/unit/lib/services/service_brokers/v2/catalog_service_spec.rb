  ...
  RSpec.describe CatalogService do
    ...
    describe '#initialize' do
      it 'defaults @plan_updateable to false' do
        ...
        service = CatalogService.new(instance_double(CloudController::ServiceBroker), attrs)
    ...
    describe 'validations' do
      ...
      context 'when the service is valid, except for duplicate plan ids' do
        let(:service) do
          ...
          CatalogService.new(instance_double(CloudController::ServiceBroker), service_attrs)
        end
      ...
      describe '#valid?' do
        let(:broker) { instance_double(CloudController::ServiceBroker) }
    ...
    describe '#cc_service' do
      ...
      context 'when a Service exists with a different service broker, but the same broker provided id' do
        let!(:cc_service) do
          CloudController::Service.make(unique_id: broker_provided_id)
        end
