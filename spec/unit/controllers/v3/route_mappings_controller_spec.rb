...
RSpec.describe RouteMappingsController, type: :controller do
  ...
  let!(:app_process) { CloudController::ProcessModel.make(:process, app: app, type: 'web', ports: [8080]) }
  ...
  describe '#create' do
    ...
    it 'successfully creates a route mapping' do
      ...
      expect(parsed_body['guid']).to eq(CloudController::RouteMappingModel.last.guid)
    end
    ...
    context 'when the mapping is invalid' do
      before do
        allow(CloudController::RouteMappingCreate).to receive(:add).and_raise(CloudController::RouteMappingCreate::InvalidRouteMapping.new('shablam'))
      end
  ...
  describe '#show' do
    let(:route_mapping) { CloudController::RouteMappingModel.make(app: app, route: route) }
    let(:user) { set_current_user(CloudController::User.make) }
  ...
  describe '#index' do
    ...
    it 'returns route mappings the user has roles to see' do
      route_mapping_1 = CloudController::RouteMappingModel.make(app: CloudController::AppModel.make(space: space))
      route_mapping_2 = CloudController::RouteMappingModel.make(app: CloudController::AppModel.make(space: space))
    ...
    context 'when accessed as an app subresource' do
      it 'uses the app as a filter' do
        route_mapping_1 = CloudController::RouteMappingModel.make(app: app)
        route_mapping_2 = CloudController::RouteMappingModel.make(app: app)
        CloudController::RouteMappingModel.make(app: CloudController::AppModel.make(space: space))
      ...
      context 'when pagination options are specified' do
        ...
        it 'paginates the response' do
          CloudController::RouteMappingModel.make(app: app)
          CloudController::RouteMappingModel.make(app: app)
    ...
    context 'permissions' do
      context 'when the user has global read access' do
        ...
        it 'lists all route_mappings' do
          route_mapping_1 = CloudController::RouteMappingModel.make(app_guid: app.guid)
          route_mapping_2 = CloudController::RouteMappingModel.make(app_guid: app.guid)
          route_mapping_3 = CloudController::RouteMappingModel.make
