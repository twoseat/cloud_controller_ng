  ...
  RSpec.describe VCAP::CloudController::AppsController do
    ...
    describe 'update app' do
      ...
      it 'updates the app' do
        ...
        put "/v2/apps/#{v3_app.guid}", MultiJson.dump(request)
        expect(last_response.status).to eq(201)
    ...
    describe 'on state change' do
      ...
      context 'when docker is disabled' do
        let!(:stopped_process) { ProcessModelFactory.make(:docker, state: 'STOPPED', docker_image: 'docker-image', type: ProcessTypes::WEB) }
        let!(:started_process) { ProcessModelFactory.make(:docker, state: 'STARTED', docker_image: 'docker-image', type: ProcessTypes::WEB) }
