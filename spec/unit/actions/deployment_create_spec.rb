...
module CloudController
  RSpec.describe DeploymentCreate do
    ...
    let!(:web_process) { CloudController::ProcessModel.make(app: app) }
    let(:droplet) { CloudController::DropletModel.make }
    let!(:route1) { CloudController::Route.make(space: app.space) }
    let!(:route_mapping1) { CloudController::RouteMappingModel.make(app: app, route: route1, process_type: web_process.type) }
    let!(:route2) { CloudController::Route.make(space: app.space) }
    let!(:route_mapping2) { CloudController::RouteMappingModel.make(app: app, route: route2, process_type: web_process.type) }
