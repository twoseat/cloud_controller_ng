...
RSpec.describe MaxAppInstancesPolicy do
  ...
  let(:process) { CloudController::ProcessModelFactory.make(instances: 1, state: 'STARTED') }
  let(:org_space) { CloudController::Space.make organization: process.organization }
  let!(:org_process) { CloudController::ProcessModelFactory.make(space: org_space, instances: current_org_instances, state: 'STARTED') }
  ...
  context 'when the app is stopped' do
    let(:process) { CloudController::ProcessModelFactory.make(instances: 1, state: 'STOPPED') }
  ...
  context 'when number of other stopped apps exceeds instance limit' do
    let!(:stopped_process) { CloudController::ProcessModelFactory.make(space: org_space, instances: 11, state: 'STOPPED') }
