...
RSpec.describe MaxAppTasksPolicy do
  let(:quota_definition) { CloudController::QuotaDefinition.make(app_task_limit: 1) }
  let(:org) { space.organization }
  let(:space) { CloudController::Space.make }
  let(:app) { CloudController::AppModel.make(space_guid: space.guid) }
  let(:task) { CloudController::TaskModel.new(app: app) }
  ...
  context 'when the quota is exceeded' do
    before do
      CloudController::TaskModel.make(app: app)
    end
