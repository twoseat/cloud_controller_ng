    ...
    RSpec.describe CefLogs do
      ...
      describe 'logging' do
        ...
        it 'logs in the expected format' do
          ...
          expect(logger).to have_received(:info).with(
            "CEF:#{cef_version}|cloud_foundry|cloud_controller_ng|#{CloudController::Constants::API_VERSION}|" \
            "#{signature_id}|#{name}|" \
            "#{severity}|" \
      ...
      describe 'CEF encoding' do
        ...
        it 'escapes "|" in the prefix' do
          ...
          expect(logger).to have_received(:info).with(
            "CEF:0|cloud_foundry|cloud_controller_ng|#{CloudController::Constants::API_VERSION}|" \
            'a\|b pa\|th|a\|b pa\|th|' \
            '0|' \
            "rt=#{(Time.now.utc.to_f * 1000).to_i} " \
            'suser=zach-loves-cake suid=some-guid ' \
