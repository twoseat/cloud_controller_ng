  ...
  describe 'GET /v3/organizations' do
    ...
    [:admin, :admin_read_only, :global_auditor].each do |role|
      it "returns all the organizations when the user is a(n) #{role}" do
        ...
        response = make_get_request('/v3/organizations?per_page=5000', http_headers(user_token(CloudController::User.make, opts)))
        expect(response.code).to eq('200')
  ...
  describe 'GET /v3/spaces' do
    ...
    [:admin, :admin_read_only, :global_auditor].each do |role|
      it "returns all the spaces when the user is a(n) #{role}" do
        ...
        # There may be > 50 results (default page size) due to pollution
        response = make_get_request('/v3/spaces?per_page=5000', http_headers(user_token(CloudController::User.make, opts)))
        expect(response.code).to eq('200')
  ...
  describe 'GET /v3/apps' do
    ...
    [:admin, :admin_read_only, :global_auditor].each do |role|
      it "returns all the spaces when the user is a(n) #{role}" do
        ...
        # There may be > 50 results (default page size) due to pollution
        response = make_get_request('/v3/apps?per_page=5000', http_headers(user_token(CloudController::User.make, opts)))
        expect(response.code).to eq('200')
