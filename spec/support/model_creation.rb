module ModelCreation
  ...
  def make_domain_for_org(org)
    CloudController::PrivateDomain.make(owning_organization: org)
  end
  ...
  def make_service_binding_for_service_instance(service_instance)
    process = CloudController::ProcessModelFactory.make(space: service_instance.space)
  ...
  def make_user_with_default_space(opts={})
    user = CloudController::User.make(admin: opts.key?(:admin), active: true)
  ...
  def make_user(opts={})
    CloudController::User.make(admin: opts.key?(:admin), active: true)
  end
end
