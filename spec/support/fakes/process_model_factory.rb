  ...
  class ProcessModelFactory
    ...
    class << self
      def make(*args)
        ...
        CloudController::ProcessModel.make(*args, process_attributes)
      end
      ...
      def make_parent_app(package_attributes, parent_app_attributes, process_attributes)
        ...
        if parent_app_attributes.any?
          ...
          parent_app = CloudController::AppModel.make(parent_app_blueprint_type, parent_app_attributes)
      ...
      def make_package(package_attributes, parent_app)
        if package_attributes.empty?
          CloudController::PackageModel.make(app: parent_app, state: PackageModel::READY_STATE, package_hash: Sham.guid)
        else
          docker_credentials = package_attributes[:docker_credentials].nil? ? {} : package_attributes[:docker_credentials]
          CloudController::PackageModel.make(:docker, app: parent_app,
